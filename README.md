...
```bash
month_end_notification$ sam build --use-container
```
...
```bash
month_end_notification$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
month_end_notification$ sam logs -n HelloWorldFunction --stack-name month_end_notification --tail
```
...
```bash
month_end_notification$ pip install -r tests/requirements.txt --user
# unit test
month_end_notification$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
month_end_notification$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name month_end_notification
```
